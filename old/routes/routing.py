...
def reportTest():
    ...
    jsonData = {}
    """
    query = session.newQuery("octobersurvey")
    query.addFilter("createdby_id", "==", 1)
    ...
    """
    def countSupportByUser(support,user):
        count = 0
        for item in support:
            if item.createdby_id == user:
                count += 1
        return count
    
    query = session.newQuery("octobersurvey")
    #query.addFilter("createdby_id", "==", 1)
    #table = query.getCountBy("id","createdby_id")
    ...
    for item in table.all():
        supportChaves = int(item.chavesScale)
        if supportChaves == 1:
            support1.append(item)
        elif supportChaves == 2:
            support2.append(item)
        elif supportChaves == 3:
            support3.append(item)
        elif supportChaves == 4:
            support4.append(item)
        elif supportChaves == 5:
            support5.append(item)
        elif supportChaves == 6:
            support6.append(item)
        elif supportChaves == 7:
            support7.append(item)
        elif supportChaves == 8:
            support8.append(item)
        elif supportChaves == 9:
            support9.append(item)
        elif supportChaves == 10:
            print("hola")
            support10.append(item)
        #print(supportChaves)
    supportGeneral.append(support1)
    supportGeneral.append(support2)
    supportGeneral.append(support3)
    supportGeneral.append(support4)
    supportGeneral.append(support5)
    supportGeneral.append(support6)
    supportGeneral.append(support7)
    supportGeneral.append(support8)
    supportGeneral.append(support9)
    supportGeneral.append(support10)
    for index, support in enumerate(supportGeneral):
        userSup = []
        for user in range(1,11):
            count = countSupportByUser(support,user)
            userSup.append(count)
        jsonData[index + 1]= userSup
    return jsonData
    """
