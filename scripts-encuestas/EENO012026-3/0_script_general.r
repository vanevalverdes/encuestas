...
RUTA_BASE_DATOS <- "C:\\Users\\vanev\\Documents\\Opol\\Opol VIII EEN - ENE26\\survey.csv"
...
# 4. Exportación
RUTA_EXPORTACION_CONSOLIDADA <- "C:\\Users\\vanev\\Documents\\Opol\\Opol VIII EEN - ENE26\\CUADRO_CONSOLIDADO_PONDERACION.csv"
...
RUTA_EXPORTACION_TABLA <- "C:\\Users\\vanev\\Documents\\Opol\\Opol VIII EEN - ENE26\\DISTRIBUCION_MUESTRA_PONDERADA_FINAL.csv"
...
RUTA_EXPORTACION_PROVINCIA_VOTO <- "C:\\Users\\vanev\\Documents\\Opol\\Opol VIII EEN - ENE26\\intencion_voto_provincia_sexo_votos_validos.csv"
...
RUTA_EXPORTACION_PROVINCIA_VOTO <- "C:\\Users\\vanev\\Documents\\Opol\\Opol VIII EEN - ENE26\\intencion_voto_diputados_provincia_sexo_votos_validos.csv"
readr::write_csv(resultados_csv_provincia_voto_valido, RUTA_EXPORTACION_PROVINCIA_VOTO)
...
RUTA_EXPORTACION_NACIONAL_VOTO <- "C:\\Users\\vanev\\Documents\\Opol\\Opol VIII EEN - ENE26\\intencion_voto_nacional_votos_validos_por_sexo.csv"
...
grafico_partido_preferente <- ggplot(resultados_partido, aes(x = Partido, y = `Voto (%)`)) +
  ...
  labs(title = "Distribución porcentual de identificación con Partido Político, enero 2026", subtitle = "Base: Total de entrevistas válidas (Ponderado)", x = NULL, y = "Porcentaje (%)", fill = "Clasificación") +
  coord_flip() +
...
grafico_apoyo_chaves <- ggplot(resultados_apoyo_chaves_gestion, aes(x = Respuesta, y = `Voto (%)`)) +
  ...
  labs(title = "Distribución porcentual de apoyo a la gestión de Rodrigo Chaves, enero 2026", subtitle = "Base: Total entrevistas Válidas (Ponderado)", x = NULL, y = "Porcentaje (%)", fill = NULL) +
  coord_flip() +
...
grafico_religion <- ggplot(resultados_religion, aes(x = Religion, y = `Porcentaje (%)`)) +
  ...
  labs(title = "Distribución porcentual por religión, enero 2026", subtitle = "Base: Total entrevistas Válidas (Ponderado)", x = NULL, y = "Porcentaje (%)", fill = NULL) +
  coord_flip() +
...
grafico_educacion <- ggplot(resultados_educacion, aes(x = Nivel_Educacion, y = `Porcentaje (%)`)) +
  ...
  labs(title = "Distribución porcentual de personas entrevistadas por Nivel Educativo, enero 2026", subtitle = "Base: Total entrevistas válidas (Ponderado)", x = NULL, y = "Porcentaje (%)", fill = NULL) +
  coord_flip() +
...
cat("\n✅ Todos los objetos de gráficos (grafico_voto_mejorado, grafico_religion, etc.) han sido generados y almacenados en el ambiente de R. Puedes imprimirlos o guardarlos manualmente.\n")
  ...
  CARPETA_EXPORTACION <- "C:/Users/vanev/Documents/Opol/Opol VIII EEN - ENE26/Graficos/"
...
# 4. Clasificar y resumir para IMPRESIÓN DE DATOS
  ...
  CARPETA_EXPORTACION <- "C:/Users/vanev/Documents/Opol/Opol VIII EEN - ENE26/Graficos/Provincias/"
