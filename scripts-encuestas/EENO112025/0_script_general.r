...
RUTA_BASE_DATOS <- "C:\\Users\\vanev\\Documents\\Opol\\Opol Segunda EEN - NOV25\\survey.csv"
...
# -----------------------------------------------------------------
# Usamos 'sexo' en lugar de 'votara' para el cálculo del DEFF
sexo_resultado <- svymean(~sexo, design = encuesta_dsn_total, na.rm = TRUE)
# 1. Intentar cálculo de DEFF 
deff_real_array <- tryCatch({
  deff(sexo_resultado) # Referencia: Varianza de la variable 'sexo'
}, warning = function(w) {
  SE_ajustado <- SE(sexo_resultado)
  varianza_compleja <- SE_ajustado^2
  
  # Se toma la proporción de Hombres o Mujeres (ej: M)
  p_hat <- as.numeric(sexo_resultado)[1] 
  n_total <- nrow(df_analisis_total) 
  
  varianza_mas <- (p_hat * (1 - p_hat)) / (n_total - 1)
  deff_manual <- varianza_compleja / varianza_mas
  return(c(deff_manual)) 
})

# Aseguramos que tenemos el valor numérico del DEFF
DEFF_VALOR <- as.numeric(deff_real_array)[1]
# 2. ASEGURAMOS que tenemos el valor numérico del DEFF
# Si deff(votara_resultado) tuvo éxito, el valor está en el primer índice.
# Si pasó por 'warning', se devolvió como vector de un elemento.
DEFF_VALOR <- as.numeric(deff_real_array)[1]

# 2. CÁLCULO DE LOS MÁRGENES DE ERROR
# ------------------------------------
n_valido = nrow(df_analisis_total) # n = 3073 entrevistas válidas
Z = 1.96 # 95% de confianza
p_max_var = 0.5

# ME 2.A: ME MÁXIMO DEL ESTUDIO (General)
# Fórmula: Z * sqrt((p(1-p) * DEFF) / n)
ME_MAXIMO_ajustado <- Z * sqrt((p_max_var * (1 - p_max_var) * DEFF_VALOR) / n_valido) * 100

# ME 2.B: ME ESPECÍFICO DE LA VARIABLE 'VOTARA' (Preservando la lógica original)
SE_votara_ajustado <- SE(votara_resultado)
ME_votara_especifico <- SE_votara_ajustado * 1.96 * 100 
...
cat("------------------------------------------------------\n")
cat("Efecto de Diseño (DEFF) (Base Total Válida - Sexo):", round(DEFF_VALOR, 3), "\n")
cat("Margen de Error MÁXIMO (95% Conf. - General): ±", round(ME_MAXIMO_ajustado, 2), "%\n")
cat("Margen de Error ESPECÍFICO (95% Conf. - Variable 'Votará'): ±", round(ME_votara_especifico, 2), "%\n")
cat("======================================================\n")
...
RUTA_EXPORTACION_TABLA <- "C:\\Users\\vanev\\Documents\\Opol\\Opol Segunda EEN - NOV25\\DISTRIBUCION_MUESTRA_PONDERADA_FINAL.csv"
...
RUTA_EXPORTACION_PROVINCIA_VOTO <- "C:\\Users\\vanev\\Documents\\Opol\\Opol Segunda EEN - NOV25\\intencion_voto_provincia_sexo_votos_validos.csv"
...
RUTA_EXPORTACION_PROVINCIA_VOTO <- "C:\\Users\\vanev\\Documents\\Opol\\Opol Segunda EEN - NOV25\\intencion_voto_diputados_provincia_sexo_votos_validos.csv"
readr::write_csv(resultados_csv_provincia_voto_valido, RUTA_EXPORTACION_PROVINCIA_VOTO)
...
RUTA_EXPORTACION_NACIONAL_VOTO <- "C:\\Users\\vanev\\Documents\\Opol\\Opol Segunda EEN - NOV25\\intencion_voto_nacional_votos_validos_por_sexo.csv"
...
categorias_especiales_apoyo <- c("No Responde", "No Sabe")
...
grafico_distribucion_bruta_provincia <- ggplot(distribucion_bruta_provincia, 
    ...
    geom_col(fill = "#D45A26", width = 0.7) +
    geom_text(aes(label = Etiqueta_Final), hjust = -0.05, size = 3.5, fontface = "bold") +
    labs(title = "Muestra Bruta (SIN PONDERAR)", x = NULL, y = "Porcentaje Bruto (%)") +
    coord_flip() +
    scale_y_continuous(limits = c(0, max(distribucion_bruta_provincia$`% Muestra`) * 1.25)) +
    theme_minimal() 
...
distribucion_ponderada_provincia_final <- df_analisis_total %>%
    ...
    mutate(
        Etiqueta_Final = paste0(round(`% Muestra`, 1), "% (n=", round(N_Ponderado), ")")
    ) %>%
    mutate(Provincia = fct_reorder(Provincia, `% Muestra`, .fun=sum, .desc = FALSE))
...
grafico_distribucion_ponderada_provincia_completo <- ggplot(distribucion_ponderada_provincia_final, 
    ...
    coord_flip() +
    scale_y_continuous(limits = c(0, max(distribucion_ponderada_provincia_final$`% Muestra`) * 1.25)) +
    theme_minimal() 


# 2. UNIFICAR LOS GRÁFICOS usando grid.arrange
# Colocar los dos gráficos uno al lado del otro
grid.arrange(
...
# print(grafico_distribucion_muestra_sexo_provincia)
# print(grafico_religion)
# print(grafico_educacion)
# print(grafico_partido_preferente)
# print(grafico_apoyo_chaves)
# print(grafico_votara)
# print(grafico_voto_presidencial)
# print(grafico_presidente_valido)
# print(grafico_voto_nunca_votaria)
...
# 4. Clasificar y resumir para IMPRESIÓN DE DATOS
  ...
  CARPETA_EXPORTACION <- "C:/Users/vanev/Documents/Opol/Opol Segunda EEN - NOV25/Graficos/Provincias/"
