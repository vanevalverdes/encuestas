...
RUTA_BASE_DATOS <- "C:\\Users\\vanev\\Documents\\Opol\\Opol Quinta EEN - DIC25\\survey.csv"
...
RUTA_EXPORTACION_TABLA <- "C:\\Users\\vanev\\Documents\\Opol\\Opol Quinta EEN - DIC25\\DISTRIBUCION_MUESTRA_PONDERADA_FINAL.csv"
...
vals_rel <- MESS::round_percent(c(respuesta_religion_ponderada, rechazo_religion_ponderada), decimals = 2)
cat(paste(" - Tasa de Respuesta Ponderada (religiones/no sabe):", vals_rel[1], "%\n"))
cat(paste(" - Tasa de Rechazo Ponderada (No Responde):", vals_rel[2], "%\n\n"))

cat("1. Base Total Válida (Pregunta Educación):\n")
vals_edu <- MESS::round_percent(c(respuesta_educacion_ponderada, rechazo_educacion_ponderada), decimals = 2)
cat(paste(" - Tasa de Respuesta Ponderada (Niveles educativos):", vals_edu[1], "%\n"))
cat(paste(" - Tasa de Rechazo Ponderada (No Responde):", vals_edu[2], "%\n\n"))
...
vals_partido <- MESS::round_percent(c(respuesta_partido_preferente_ponderada, rechazo_partido_preferente_ponderada), decimals = 2)
cat(paste("  - Tasa de Respuesta Ponderada (Partido/Ninguno):", vals_partido[1], "%\n"))
cat(paste("  - Tasa de Rechazo Ponderada (No Responde):", vals_partido[2], "%\n"))

cat(" - Apoyo a Chaves (Base Total Válida):\n")
vals_apoyo <- MESS::round_percent(c(respuesta_apoyo_chaves_ponderada, rechazo_apoyo_chaves_ponderada), decimals = 2)
cat(paste("  - Tasa de Respuesta Ponderada (Si/No/No Sabe):", vals_apoyo[1], "%\n"))
cat(paste("  - Tasa de Rechazo Ponderada (No Responde):", vals_apoyo[2], "%\n"))
...
vals_nunca <- MESS::round_percent(c(respuesta_nunca_votaria_ponderada, rechazo_nunca_votaria_ponderada), decimals = 2)
cat(paste("  - Tasa de Respuesta Ponderada (Candidato/Nulo/Blanco/NS):", vals_nunca[1], "%\n"))
cat(paste("  - Tasa de Rechazo Ponderada (No Responde):", vals_nunca[2], "%\n"))
...
RUTA_EXPORTACION_PROVINCIA_VOTO <- "C:\\Users\\vanev\\Documents\\Opol\\Opol Quinta EEN - DIC25\\intencion_voto_provincia_sexo_votos_validos.csv"
...
RUTA_EXPORTACION_PROVINCIA_VOTO <- "C:\\Users\\vanev\\Documents\\Opol\\Opol Quinta EEN - DIC25\\intencion_voto_diputados_provincia_sexo_votos_validos.csv"
readr::write_csv(resultados_csv_provincia_voto_valido, RUTA_EXPORTACION_PROVINCIA_VOTO)
...
RUTA_EXPORTACION_NACIONAL_VOTO <- "C:\\Users\\vanev\\Documents\\Opol\\Opol Quinta EEN - DIC25\\intencion_voto_nacional_votos_validos_por_sexo.csv"
...
grafico_voto_nunca_votaria <- ggplot(resultados_nunca_candidatos, aes(x = Candidato, y = `Voto (%)`)) +
  ...
  geom_text(aes(label = paste0(`Voto (%)`, "%")), hjust = -0.1, size = 4, color = "black", fontface = "bold") +
  scale_fill_manual(values = c("Persona candidata" = "#1d1c49", "Ninguna persona definida" = "#D45A26")) +
  labs(title = "Distribución porcentual de la opción que nunca votaría a presidencia, diciembre2025", subtitle = "Base: Total de personas que votarán (Ponderado)", x = NULL, y = "Porcentaje (%)", fill = "Tipo de Voto") +
  coord_flip() +
...
grafico_partido_preferente <- ggplot(resultados_partido, aes(x = Partido, y = `Voto (%)`)) +
  ...
  labs(title = "Distribución porcentual de identificación con Partido Político, diciembre2025", subtitle = "Base: Total de entrevistas válidas (Ponderado)", x = NULL, y = "Porcentaje (%)", fill = "Clasificación") +
  coord_flip() +
...
grafico_apoyo_chaves <- ggplot(resultados_apoyo_chaves_gestion, aes(x = Respuesta, y = `Voto (%)`)) +
  ...
  labs(title = "Distribución porcentual de apoyo a la gestión de Rodrigo Chaves, diciembre2025", subtitle = "Base: Total entrevistas Válidas (Ponderado)", x = NULL, y = "Porcentaje (%)", fill = NULL) +
  coord_flip() +
...
grafico_voto_diputado <- ggplot(resultados_diputado, aes(x = Partido, y = `Voto (%)`)) +
  ...
  labs(title = "Distribución porcentual de intención de voto para diputaciones, diciembre2025", subtitle = "Base: Total de personas que votarán (Ponderado)", x = NULL, y = "Porcentaje de Voto (%)", fill = "Clasificación") +
  coord_flip() +
...
grafico_votara <- ggplot(resultados_votara, aes(x = Respuesta, y = `Voto (%)`)) +
  ...
  labs(title = "Distribución porcentual de decididos a votar en próximas elecciones, diciembre2025", subtitle = "Base: Total entrevistas Válidas (Ponderado)", x = NULL, y = "Porcentaje (%)", fill = NULL) +
  coord_flip() +
...
resultados_religion <- religion_df %>%
  ...
  mutate(`Porcentaje (%)` = MESS::round_percent(Proporcion, decimals = 2)) %>%
...
grafico_religion <- ggplot(resultados_religion, aes(x = Religion, y = `Porcentaje (%)`)) +
  ...
  labs(title = "Distribución porcentual por religión, diciembre2025", subtitle = "Base: Total entrevistas Válidas (Ponderado)", x = NULL, y = "Porcentaje (%)", fill = NULL) +
  coord_flip() +
...
grafico_educacion <- ggplot(resultados_educacion, aes(x = Nivel_Educacion, y = `Porcentaje (%)`)) +
  ...
  labs(title = "Distribución porcentual de personas entrevistadas por Nivel Educativo, diciembre2025", subtitle = "Base: Total entrevistas válidas (Ponderado)", x = NULL, y = "Porcentaje (%)", fill = NULL) +
  coord_flip() +
...
grafico_distribucion_muestra_sexo_provincia <- ggplot(distribucion_ponderada, 
    ...
    labs(
        title = "Distribución porcentual de personas entrevistadas por provincia y sexo, diciembre2025",
...
grafico_presidente_valido <- ggplot(df_partidos, aes(x = Partido, y = `Voto (%)`)) +
  ...
  labs(
    title = "Distribución porcentual de personas entrevistadas por Intención de Voto Válido a presidencia, diciembre2025",
...
grafico_escala_voto <- ggplot(df_escala, aes(x = Escala_Nivel, y = `Voto (%)`)) +
    ...
    geom_text(aes(label = paste0(`Voto (%)`, "%")), 
              hjust = -0.1, size = 4.0, color = "black", fontface = "bold") +
    labs(
        title = "Distribución porcentual de entrevistas por escala de decididos a votar, diciembre2025",
...
cat("\n✅ Todos los objetos de gráficos (grafico_voto_mejorado, grafico_religion, etc.) han sido generados y almacenados en el ambiente de R. Puedes imprimirlos o guardarlos manualmente.\n")
  ...
  CARPETA_EXPORTACION <- "C:/Users/vanev/Documents/Opol/Opol Quinta EEN - DIC25/Graficos/"
...
# 4. Clasificar y resumir para IMPRESIÓN DE DATOS
  ...
  CARPETA_EXPORTACION <- "C:/Users/vanev/Documents/Opol/Opol Quinta EEN - DIC25/Graficos/Provincias/"
